...
if (!isset($boxes)) { // auto_create_done
    ...
    // Nex 130525
    $file = fopen(SM_PATH. 'local_data/test/offline_sent_box', "r");
    if (flock($file, LOCK_SH)) {
        $string = fread($file, 8192);
        flock($file, LOCK_UN);
    }
    fclose($file);
    eval('$offline_sent_box = '. $string. ';');
    $file = fopen(SM_PATH. 'local_data/test/offline_draft_box', "r");
    if (flock($file, LOCK_SH)) {
        $string = fread($file, 8192);
        flock($file, LOCK_UN);
    }
    fclose($file);
    eval('$offline_draft_box = '. $string. ';');
    array_unshift($boxes, $offline_sent_box, $offline_draft_box);
    // Nex
}
...
// Test 130525
$file = fopen(SM_PATH. 'local_data/test/mailbox', "w+");
fwrite($file, var_export($boxes, true));
fclose($file);
// Test
