...
if ($draft) {
     ...
     */
    set_my_charset();
    if (! offlineDeliverMessage($composeMessage, true)) {
...
if ($send) {
    ...
    if (checkInput(false) && !isset($AttachFailure)) {
        ...
        $Result = offlineDeliverMessage($composeMessage);
 ...
 *
 */
function offlineDeliverMessage(&$composeMessage, $draft=false) {
    ...
    global $offline_sent_num, $offline_draft_num;
    ...
    $composeMessage->rfc822_header = $rfc822_header;
    // Nex 130526
    //$file = fopen("../offline/temp", "r");
    //$offline_sent_num = fgets($file);
    //$offline_sent_num = 0;
    //$offline_draft_num = fgets($file);
    //$offline_draft_num = 0;
    $nums = file("../local_data/temp", FILE_IGNORE_NEW_LINES | FILE_IGNORE_NEW_LINES);
    $offline_sent_num = $nums[0];
    $offline_draft_num = $nums[1];
    //fclose($file);
    // Nex 130528
    $file = fopen(SM_PATH. OFFLINE_SENT. $offline_sent_num, "w+");
    // Test 130528
    $composeMessage->type0 = $content_type->type0;
    $composeMessage->type1 = $content_type->type1;
    $composeMessage->entity_id = '1';
    if (flock($file, LOCK_EX)) {
        fwrite($file, serialize($composeMessage));
        flock($file, LOCK_UN);
    }
    fclose($file);
    $composeMessage->type0 = '';
    $composeMessage->type1 = '';
    $composeMessage->entity_id = '';
    // Test
    // Nex 130526
    $file = fopen(SM_PATH. 'local_data/test/empty_msg', "r");
    ...
    fclose($file);
    eval('$msg = '. $string. ';');
    $msg['ID'] = $offline_sent_num;
    $msg['FROM'] = $from_mail;
    $msg['SUBJECT'] = $subject;
    $msg['TO'] = $send_to;
    $msg['PRIORITY'] = $mailprio;
    $msg['CC'] = $send_to_cc;
    $msg['TYPE0'] = $content_type->type0;
    $msg['TYPE1'] = $content_type->type1;
    $msg['FROM-SORT'] = $from_mail;
    $msg['SUBJECT-SORT'] = $subject;
    $msg['TO-SORT'] = $send_to;
    //$file = fopen(SM_PATH. OFFLINE_SENT. 'msgs/'. $offline_sent_num, "w+");
    //$offline_sent_num++;
    //fwrite($file, var_export($msg, true));
    //fclose($file);
    // Nex 130527
    if ($offline_sent_num == 0) {
        $file = fopen(SM_PATH. 'local_data/test/empty_msgs', "r");
    } else {
        $file = fopen(SM_PATH. OFFLINE_SENT. 'msgs', "r");
    }
    if (flock($file, LOCK_SH)) {
        $string = fread($file, 8192);
        flock($file, LOCK_UN);
    ...
    $file = fopen(SM_PATH. OFFLINE_SENT. 'msgs', "w+");
    if (flock($file, LOCK_EX)) {
    ...
    $offline_sent_num++;
    // Nex 130526
    $file = fopen(SM_PATH. "local_data/temp", "w+");
    if (flock($file, LOCK_EX)) {
        fwrite($file, $offline_sent_num. "\r\n". $offline_draft_num);
        flock($file, LOCK_UN);
    }
    fclose($file);
    // Nex 130601
    ...
    $instr = new Instr();
    $instr->instr_name = 'syncDeliverMessage';
    $instr->param_list = array($composeMessage, $draft, $action, $passed_id, $passed_ent_id, $useSendmail, $domain, $mailbox);
    $instr->append();
    // Nex
