...
function formatBody($imap_stream, $message, $color, $wrap_at, $ent_num, $id, $mailbox='INBOX',$clean=false) {
    ...
    if (($body_message->header->type0 == 'text') ||
            ...
            // Nex 130528
            $mailbox == 'Offline Sent' || $mailbox == 'Offline Drafts') {
        // Nex 130528
        if ($mailbox == 'Offline Sent' || $mailbox == 'Offline Drafts') {
            $body = $message->body_part;
            $body = decodeBody($body, $message->rfc822_header->encoding);
        // Nex
        } else {
        ...
        if ($body_message->header->type1 == 'html' ||
            // Nex 130528
            (($mailbox == 'Offline Sent' || $mailbox == 'Offline Drafts') && $message->type1 == 'html')) {
            // Nex
            if ($show_html_default <> 1) {
                ...
                // Nex 130528
                if ($mailbox == 'Offline Sent' || $mailbox == 'Offline Drafts') {
                    translateText($body, $wrap_at, $message->rfc822_header->content_type->properties->charset);
                } else {
                // Nex
                ...
                // Nex 130528
                if ($mailbox == 'Offline Sent' || $mailbox == 'Offline Drafts') {
                    $charset = $message->rfc822_header->content_type->properties->charset;
                } else {
                // Nex
                ...
                $body = magicHTML($body, $id, $message, $mailbox); // Nex TODO
            }
        } else {
            // Nex 130528
            if ($mailbox == 'Offline Sent' || $mailbox == 'Offline Drafts') {
